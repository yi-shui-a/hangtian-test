...
DiscretePoint simulateMissileEvent(uint32_t time_ms)
{
    ...
    if (time_ms <= STEP_B)
    {
        double t_norm = time / (STEP_B / 1000);
        ...
        // 速度计算（动力加速）
        double thrust_factor = (point.event == TLJCYQRXH || point.event == EJFDJDHXH) ? 1.5 : 1.0;
        ...
        // 加速度（含级间分离冲击）
        if (point.event == FSTFLXH || point.event == ZLZFLXH)
        {
    ...
    // 巡航阶段（80-380秒）：弹翼展开+机动
    else if (time_ms <= STEP_C)
    {
        double cruiseTime = time - (STEP_B / 1000);
        double phase = cruiseTime / 30 * 2 * M_PI;

        // 姿态动力学（弹翼展开效应）
        double wingEffect = (point.event >= DTMCJSXH) ? 1.5 : 1.0;
        ...
        // 位置计算（蛇形机动）
        point.latitude = 39.9042 + 0.5 * cruiseTime / 240 + 0.05 * sin(phase) * (point.event == YESFXH ? 2.0 : 1.0) + perturb * 1e-5;
        point.longitude = 116.4074 + 0.8 * cruiseTime / 240 + 0.03 * cos(0.8 * phase) * (point.event == YESFXH ? 1.5 : 1.0) + perturb * 1e-5;
        point.altitude = 10000.0 + 50 * sin(0.5 * phase) + perturb * 0.8;

        // 速度计算（干扰弹效应）
        double jammerEffect = (point.event == YESFXH) ? 1.2 : 1.0;
    ...
    {
        double attackTime = time - (STEP_C / 1000);
        double t_norm = attackTime / ((STEP_D - STEP_C) / 1000);
        ...
        // 位置计算（制导螺旋）
        point.latitude = 40.4042 - 0.2 * t_norm + 0.02 * sin(spiralPhase) * (point.event == ZRZTTZXH ? 2.0 : 1.0) + perturb * 1e-5;
        point.longitude = 117.2074 - 0.1 * t_norm + 0.02 * cos(spiralPhase) * (point.event == MDZDXZXH ? 1.5 : 1.0) + perturb * 1e-5;
        point.altitude = 10000.0 * (1 - pow(t_norm, 1.5)) + 100 * sin(2 * spiralPhase) + perturb * 1.2;

        // 速度计算（末端加速）
        double guidanceEffect = (point.event >= ZRZTTZXH) ? 1.8 : 1.0;
